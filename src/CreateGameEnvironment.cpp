...
void initializeGhosts(std::vector<Ghost> &ghosts, int numberOfGhosts, std::vector<std::shared_ptr<Street>> &streets){
    ...
    for (int i=0; i<numberOfGhosts; i++) {
        ghosts.emplace_back(Ghost(ghostColor[i],streets.at(ghostStreetIndex[i]),
